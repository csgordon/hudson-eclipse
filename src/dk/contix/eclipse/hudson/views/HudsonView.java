...
@UIType @UI public class HudsonView extends ViewPart implements @UI PropertyChangeListener {
	...
	private void configurePreferences() {
		...
		prefs.addPropertyChangeListener(new Preferences.IPropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent event) {
				...
				/* MAYBE -- Colin Gordon */ refreshAction.run(); // Colin Gordon: This MAY be a bug: running in IPropertyChangeListener, which may run on any thread.
			}
		});
		baseUrl = prefs.getString(Activator.PREF_BASE_URL);
	}
	...
	private void makeActions() {
		scheduleAction = new Action() {
			public void run() {
				...
				sj.schedule(); // XXX: What queue does this go onto?  Probably requires @AlwaysSafe receiver permission!
			}
		};
	...
	@SafeEffect public void refreshTableViewer() {
		refreshAction.run();
	}
